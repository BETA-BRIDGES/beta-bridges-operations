...
export type AppJob={id:string;jobId:string;client:string;clientId:string|null;vehicles:number;date:string;time:string;technician:string;technicianId:string|null;status:string;location:string};
...
type JobLookupRow={id:string;job_id:string;client_id:string|null;number_of_vehicles:number;scheduled_date:string;scheduled_time:string|null;assigned_technician_id:string|null;status:string;location:string|null};
...
export async function loadJobs():Promise<AppJob[]>{
  if(!supabase) return [];
  const {data,error}=await supabase.from("jobs").select("id,job_id,client_id,number_of_vehicles,scheduled_date,scheduled_time,assigned_technician_id,status,location").order("scheduled_date",{ascending:true});
  ...
  return rows.map(j=>({id:j.id,jobId:j.job_id,client:clientMap.get(j.client_id ?? "")||"—",clientId:j.client_id||null,vehicles:j.number_of_vehicles,date:formatDate(j.scheduled_date),time:j.scheduled_time??"",technician:techMap.get(j.assigned_technician_id ?? "")||"Unassigned",technicianId:j.assigned_technician_id||null,status:j.status,location:j.location||""}));
}
