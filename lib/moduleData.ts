...
export type ChargeRecord={id:string;chargeId:string;client:string;location:string;logistics:number;accommodation:number;swap:number;deinstallation:number;reinstallation:number;healthCheck:number;simReplacement:number;others:number;status:string};
...
export type WeeklyRecord={id:string;technician:string;week:string;projects:number;vehiclesCompleted:number;date:string;remarks:string};

function safeDate(value:string|null){return value??"";}
function num(value:number|null){return Number(value??0);}
...
export async function loadCharges():Promise<ChargeRecord[]>{
  ...
  return (data??[]).map(c=>({id:c.id,chargeId:c.charge_id,client:map.get(c.client_id)||"—",location:c.location??"",logistics:num(c.logistics),accommodation:num(c.accommodation),swap:num(c.swap),deinstallation:num(c.deinstallation),reinstallation:num(c.reinstallation),healthCheck:num(c.health_check),simReplacement:num(c.sim_replacement),others:num(c.others),status:c.paid_or_approved??"Pending"}));
}
...
export async function createCharge(input:{clientId?:string|null;location?:string;logistics?:number;accommodation?:number;swap?:number;deinstallation?:number;reinstallation?:number;healthCheck?:number;simReplacement?:number;others?:number},role:Role){
  ...
  const {data:latest}=await supabase.from("miscellaneous_charges").select("charge_id").like("charge_id",`CHG-${stamp}-%`).order("charge_id",{ascending:false}).limit(1);
