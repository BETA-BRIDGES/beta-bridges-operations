...
export default function Home(){
  ...
  return <div className="app">
    ...
    <main className="main"><header className="topbar"><div><h1 className="page-title">{module}</h1><div className="muted">Central operations workspace</div></div><div className="topbar-actions">{supabase?<div className="user-chip"><strong>{profile?.full_name||profile?.email}</strong><span>{role}</span></div>:<select value={demoRole} onChange={e=>{setDemoRole(e.target.value as Role);setModule("Dashboard")}} className="role-select">{ROLES.map(r=><option key={r}>{r}</option>)}</select>}{supabase&&<button className="btn" onClick={signOut}>Sign out</button>}</div></header>
      ...
      {module==="Vehicle Management"&&<section className="card"><div className="section-head"><div><h3>Vehicle Management</h3><p className="muted">Vehicle records belong to an Operational Job. One project can contain multiple vehicles; Daily Job Done and Used Stock must use the same Job + Vehicle link.</p></div>{canCreate(role,module)&&<button className="btn primary" onClick={openVehicleCreate}>Add vehicle</button>}</div><section className="grid"><div className="card"><div className="muted">Total vehicles</div><div className="stat">{managedVehicles.length}</div></div><div className="card"><div className="muted">Pending</div><div className="stat">{vehiclePending}</div></div><div className="card"><div className="muted">In Progress</div><div className="stat">{vehicleInProgress}</div></div><div className="card"><div className="muted">Completed</div><div className="stat">{vehicleCompleted}</div></div></section><div className="section-head"><input className="table-search" value={tableSearch} onChange={e=>{setTableSearch(e.target.value);setTablePage(1)}} placeholder="Search registration, Job ID, client, make, technician…"/><span className="muted">{filteredVehicles.length} matching</span></div><Table headers={["Job ID","Client","Registration","Vehicle Make","Vehicle Details","Technician","Status","Actions"]}>{paginate(filteredVehicles).map(v=><tr key={v.id}><td>{v.jobKey||"—"}</td><td>{v.client||"—"}</td><td><strong>{v.registration||"Unregistered"}</strong></td><td>{v.vehicleMake||"—"}</td><td>{v.vehicleDetails||"—"}</td><td>{v.technician||"—"}</td><td>{v.status}</td><td>{can(role,module,"edit")&&<button className="btn small" onClick={()=>openVehicleEdit(v)}>Edit</button>}</td></tr>)}</Table><Pager page={tablePage} total={filteredVehicles.length} pageSize={pageSize} onPage={setTablePage}/></section></section>
    ...
    {modal&&<div className="modal-backdrop"><form className="modal" onSubmit={saveRecord}>
      <div className="section-head"><div><h3>{formMode==="reminder"?"Create reminder":formMode==="create-user"?"Create user account":formMode==="user"?"Edit user access":formMode==="comment"?"Task comment":formMode==="remark"?"Daily Job Done remark":formMode==="vehicles"?"Vehicle details":formMode==="vehicle-record"?(editing?"Edit vehicle":"Add vehicle"):editing?`Edit ${module}`:`Create ${module}`}</h3><p className="muted">Changes are enforced against the assigned role in Supabase.</p></div><button type="button" className="btn" onClick={closeModal}>Close</button></div>
