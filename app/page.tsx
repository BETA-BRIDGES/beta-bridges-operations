...
import { ClientRecord, ChargeRecord, CompletionRecord, NotificationRecord, StockRecord, UserRecord, WeeklyRecord, addCompletionRemark, createCharge, createClient, createCompletion, createStock, loadCharges, loadClients, loadCompletions, loadNotifications, loadProfiles, loadStock, loadWeekly, markNotificationRead, updateCharge, updateClient, updateStock, updateUserProfile, upsertWeekly } from "../lib/moduleData";
...
export default function Home(){
  ...
  return <div className="app">
    ...
    <main className="main"><header className="topbar"><div><h1 className="page-title">{module}</h1><div className="muted">Central operations workspace</div></div><div className="topbar-actions">{supabase?<div className="user-chip"><strong>{profile?.full_name||profile?.email}</strong><span>{role}</span></div>:<select value={demoRole} onChange={e=>{setDemoRole(e.target.value as Role);setModule("Dashboard")}} className="role-select">{ROLES.map(r=><option key={r}>{r}</option>)}</select>}{supabase&&<button className="btn" onClick={signOut}>Sign out</button>}</div></header>
      ...
      {module==="Administration"&&role==="Super Admin"&&<section className="section two"><div className="card"><h3>Users & Roles</h3><Table headers={["Name","Email","Role","Status","Actions"]}>{users.map(u=><tr key={u.id}><td>{u.fullName}</td><td>{u.email}</td><td>{u.role}</td><td>{u.active?"Active":"Inactive"}</td><td><button className="btn small" onClick={()=>openUserEdit(u)}>Edit access</button></td></tr>)}</Table></div><div className="card"><h3>Google Sheets Connections</h3><p className="muted">The six legacy spreadsheet IDs and mappings are stored in the project. OAuth authorization is the remaining external Google step.</p><span className="badge warn">Google authorization pending</span></div></section>}
    ...
    {modal&&<div className="modal-backdrop"><form className="modal" onSubmit={saveRecord}>
      <div className="section-head"><div><h3>{formMode==="user"?"Edit user access":formMode==="comment"?"Task comment":formMode==="remark"?"Daily Job Done remark":editing?`Edit ${module}`:`Create ${module}`}</h3><p className="muted">Changes are enforced against the assigned role in Supabase.</p></div><button type="button" className="btn" onClick={closeModal}>Close</button></div>
