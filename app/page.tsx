...
import { ClientRecord, ChargeRecord, CompletionRecord, NotificationRecord, ReminderRecord, StockRecord, UserRecord, WeeklyRecord, addCompletionRemark, updateCompletion, createCharge, createClient, createCompletion, createStock, loadCharges, loadClients, loadCompletions, loadNotifications, loadProfiles, loadStock, loadReminders, loadWeekly, markNotificationRead, markReminderSent, updateCharge, updateClient, updateStock, updateUserProfile, upsertWeekly, createReminder } from "../lib/moduleData";
...
export default function Home(){
  ...
  function openUserEdit(user:UserRecord){setModule("Administration");setEditing(true);setFormMode("user");setSelectedId(user.id);setForm({role:user.role,active:String(user.active)});setModal(true)}
  ...
  function openEdit(target:string,record:any){
    ...
    if(target==="Daily Job Done") setForm({jobId:record.jobUuid||"",deviceId:record.deviceId,date:record.date||today(),installer:record.installer,location:record.location,client:record.client,vehicleDetails:record.vehicleDetails,vehicleMake:record.vehicleMake,tssOfficer:record.tssOfficer,remarks:record.remarks});
  ...
  async function saveRecord(e:FormEvent){
    ...
    try{
      if(formMode==="reminder"){
        ...
        await updateUserProfile(selectedId,{role:form.role as Role,active:form.active==="true"},role);
        ...
        else if(editing) await updateCompletion(selectedId,{jobId:form.jobId||null,deviceId:form.deviceId,date:form.date,installer:form.installer,location:form.location,client:form.client,vehicleDetails:form.vehicleDetails,vehicleMake:form.vehicleMake,tssOfficer:form.tssOfficer,remarks:form.remarks},role);
  ...
  const filteredJobs=jobs.filter(j=>matches([j.jobId,j.client,j.tssOfficer,j.technician,j.location,j.status,j.date]));
  ...
  const filteredStock=stock.filter(s=>matches([s.deviceId,s.simId,s.dateIssued,s.dateInstalled,s.installer,s.client,s.location,s.network,s.deviceType,s.deviceStatus]));
  const filteredWeekly=weekly.filter(w=>matches([w.technician,w.week,w.projects,w.vehiclesCompleted,w.date,w.remarks]));
  ...
  const filteredTasks=tasks.filter(t=>matches([t.taskKey,t.title,t.assignee,t.due,t.status]));
  ...
  return <div className="app">
    ...
    {modal&&<div className="modal-backdrop"><form className="modal" onSubmit={saveRecord}>
      <div className="section-head"><div><h3>{formMode==="reminder"?"Create reminder":formMode==="user"?"Edit user access":formMode==="comment"?"Task comment":formMode==="remark"?"Daily Job Done remark":editing?`Edit ${module}`:`Create ${module}`}</h3><p className="muted">Changes are enforced against the assigned role in Supabase.</p></div><button type="button" className="btn" onClick={closeModal}>Close</button></div>
      ...
      {formMode==="reminder"?<div className="form-grid"><Field label="Title" name="title" value={form.title||""} setValue={v=>setField("title",v)}/><Field label="Assigned user" name="userId" value={form.userId||""} setValue={v=>setField("userId",v)} options={users.filter(u=>u.active).map(u=>({value:u.id,label:`${u.fullName} · ${u.role}`}))}/><Field label="Remind at" name="remindAt" type="datetime-local" value={form.remindAt||""} setValue={v=>setField("remindAt",v)}/><Field label="Details" name="details" type="textarea" value={form.details||""} setValue={v=>setField("details",v)}/></div>:formMode==="user"?<div className="form-grid"><Field label="Role" name="role" value={form.role||""} setValue={v=>setField("role",v)} options={ROLES.map(r=>({value:r,label:r}))}/><Field label="Account status" name="active" value={form.active||"true"} setValue={v=>setField("active",v)} options={[{value:"true",label:"Active"},{value:"false",label:"Inactive"}]}/></div>:formMode==="remark"?<div className="form-grid"><Field label="Remark" name="remark" type="textarea" value={form.remark||""} setValue={v=>setField("remark",v)} /></div>:formMode==="comment"?<div className="form-grid"><Field label="Comment" name="comment" type="textarea" value={form.comment||""} setValue={v=>setField("comment",v)} /></div>:<>
