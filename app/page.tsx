...
export default function Home(){
  ...
  const [jobListingDateMode,setJobListingDateMode]=useState<"selected"|"all">("selected");
  const [jobListingSelectedDate,setJobListingSelectedDate]=useState(today());
  const [jobDoneDateMode,setJobDoneDateMode]=useState<"selected"|"all">("selected");
  const [jobDoneSelectedDate,setJobDoneSelectedDate]=useState(today());
  const [weeklyTechnicianFilter,setWeeklyTechnicianFilter]=useState("all");
  const [legacyExceptionStatusFilter,setLegacyExceptionStatusFilter]=useState("Open");
  const [legacyExceptionTypeFilter,setLegacyExceptionTypeFilter]=useState("all");
  const pageSize=50;
  ...
  function openJobTask(job:AppJob){
    setModule("Tasks");
    setTableSearch(job.jobId);
    setTablePage(1);
  }
  ...
  const filteredJobs=visibleJobs.filter(j=>{
    ...
    const dateMatch=jobListingDateMode==="all" || !jobListingSelectedDate || j.dateIso===jobListingSelectedDate;
    return statusMatch && assignmentMatch && dateMatch && matches([j.jobId,j.client,j.tssOfficer,j.technician,j.location,j.status,j.date,j.priority]);
  });
  const filteredCompletions=completions.filter(c=>{
    const dateMatch=jobDoneDateMode==="all" || !jobDoneSelectedDate || c.date===jobDoneSelectedDate;
    return dateMatch && matches([c.jobId,c.jobUuid,c.vehicleId,c.deviceId,c.date,c.installer,c.client,c.location,c.vehicleDetails,c.vehicleMake,c.status,c.tssOfficer,c.remarks]);
  });
  ...
  const filteredWeekly=visibleWeekly.filter(w=>{
    const technicianMatch=weeklyTechnicianFilter==="all" || w.technicianId===weeklyTechnicianFilter;
    return technicianMatch && matches([w.technician,w.week,w.projects,w.vehiclesCompleted,w.date,w.remarks]);
  });
  const filteredLegacyExceptions=legacyExceptions.filter(x=>{
    const statusMatch=legacyExceptionStatusFilter==="all" || x.status===legacyExceptionStatusFilter;
    const typeMatch=legacyExceptionTypeFilter==="all" || x.exceptionType===legacyExceptionTypeFilter;
        ...
        <section className="card">
          ...
          <div className="section-head">
            <div className="modal-actions">
              <select value={jobListingDateMode} onChange={e=>{const value=e.target.value as "selected"|"all";setJobListingDateMode(value);setTablePage(1)}} aria-label="Job listing date view">
                <option value="selected">Selected date</option>
                <option value="all">All dates</option>
              </select>
              {jobListingDateMode==="selected"&&<>
                <button type="button" className="btn small" onClick={()=>{setJobListingSelectedDate(shiftDate(jobListingSelectedDate,-1));setTablePage(1)}}>Previous day</button>
                <input type="date" value={jobListingSelectedDate} onChange={e=>{setJobListingSelectedDate(e.target.value);setTablePage(1)}} aria-label="Selected job listing date"/>
                <button type="button" className="btn small" onClick={()=>{setJobListingSelectedDate(shiftDate(jobListingSelectedDate,1));setTablePage(1)}}>Next day</button>
              </>}
            </div>
          ...
          <Table headers={["Job ID","Vehicles","Client","Date","Time","Location","Technician","Status","Actions"]}>{paginate(filteredJobs).map(j=><tr key={j.id}>
            ...
            <td>{can(role,module,"edit")&&<button className="btn small" onClick={()=>openEdit(module,j)}>Edit</button>}{can(role,module,"assign")&&<button className="btn small" onClick={()=>openAssignment(j)}>{j.technicianId?"Reassign":"Assign"}</button>}{j.technicianId&&tasks.some(t=>t.relatedJobId===j.id)&&<button className="btn small" onClick={()=>openJobTask(j)}>Task</button>}{canCreate(role,module)&&<button className="btn small" onClick={()=>openVehicles(j)}>Vehicles</button>}</td>
          </tr>)}</Table>
...
{module==="Field Technician"&&<section className="section two">
      ...
      {module==="Tasks"&&<section className="card">
        <div className="section-head">
          <div><h3>Technician Tasks</h3><p className="muted">{role==="Field Technician"?"Only tasks assigned to you are shown.":"A Job assignment automatically creates one linked Technician Task. Use this workspace for acknowledgement, progress, completion and comments."}</p></div>
          {canCreate(role,module)&&<button className="btn primary" onClick={()=>openCreate(module)}>Create task</button>}
        </div>
        <div className="section-head">
          <input className="table-search" value={tableSearch} onChange={e=>{setTableSearch(e.target.value);setTablePage(1)}} placeholder="Search task, Job ID, assignee, due date…"/>
          <span className="muted">{filteredTasks.length} matching</span>
        </div>
        <Table headers={["Task ID","Linked Job","Title","Assignee","Due","Priority","Status","Actions"]}>{paginate(filteredTasks).map(t=><tr key={t.id}>
          <td>{t.taskKey}</td>
          <td>{t.relatedJobId?(jobs.find(j=>j.id===t.relatedJobId)?.jobId||"Linked job"):"—"}</td>
          <td>{t.title}</td>
          <td>{t.assignee}</td>
          <td>{t.due}</td>
          <td>{jobs.find(j=>j.id===t.relatedJobId)?.priority||"—"}</td>
          <td>{t.status}</td>
          <td>{can(role,module,"complete")&&t.status==="Pending"&&<button className="btn small" onClick={()=>taskAction(t,"Acknowledged")}>Acknowledge</button>}{can(role,module,"complete")&&t.status==="Acknowledged"&&<button className="btn small" onClick={()=>taskAction(t,"In Progress")}>Start</button>}{can(role,module,"complete")&&t.status==="In Progress"&&<button className="btn small" onClick={()=>taskAction(t,"Completed")}>Complete</button>}{can(role,module,"comment")&&<button className="btn small" onClick={()=>showComments(t)}>Comment</button>}</td>
        </tr>)}</Table>
        <Pager page={tablePage} total={filteredTasks.length} pageSize={pageSize} onPage={setTablePage}/>
      </section>}
    ...
    {modal&&<div className="modal-backdrop"><form className="modal" onSubmit={saveRecord}>
      ...
      {formMode==="assignment"?<div>
        ...
        {module==="Tasks"&&<div className="form-grid">
          <Field label="Title" name="title" value={form.title||""} setValue={v=>setField("title",v)}/>
          <Field label="Linked Job" name="relatedJobId" value={form.relatedJobId||""} setValue={v=>{
            setField("relatedJobId",v);
            const job=jobs.find(j=>j.id===v);
            if(job?.technicianId) setField("assignedTo",job.technicianId);
            if(job?.dateIso) setField("dueAt",job.dateIso+"T"+(job.time||"17:00").slice(0,5));
            if(job?.description) setField("description",job.description);
            if(job?.client&&job.client!=="—") setField("title",form.title||("Technician Job: "+job.jobId+" · "+job.client));
          }} options={jobs.map(j=>({value:j.id,label:j.jobId+" · "+j.client+" · "+j.date}))}/>
          <Field label="Assignee" name="assignedTo" value={form.assignedTo||""} setValue={v=>setField("assignedTo",v)} options={[{value:"",label:"Unassigned"},...users.filter(u=>u.active&&u.role!=="Viewer").map(u=>({value:u.id,label:`${u.fullName} · ${u.role}`}))]}/>
          <Field label="Department" name="department" value={form.department||""} setValue={v=>setField("department",v)}/>
          <Field label="Priority" name="priority" value={form.priority||"Normal"} setValue={v=>setField("priority",v)} options={["Normal","High","Urgent"].map(x=>({value:x,label:x}))}/>
          <Field label="Due" name="dueAt" type="datetime-local" value={form.dueAt||""} setValue={v=>setField("dueAt",v)}/>
          <Field label="Description" name="description" type="textarea" value={form.description||""} setValue={v=>setField("description",v)}/>
        </div>}
      </>}
