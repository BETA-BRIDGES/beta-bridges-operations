...
import { ClientRecord, ChargeRecord, CompletionRecord, NotificationRecord, ReminderRecord, StockRecord, UserRecord, VehicleRecord, WeeklyRecord, addCompletionRemark, updateCompletion, createCharge, createClient, createCompletion, createStock, createVehicle, loadCharges, loadClients, loadCompletions, loadNotifications, loadProfiles, loadStock, loadReminders, loadVehicles, loadWeekly, markNotificationRead, markReminderSent, updateCharge, updateClient, updateStock, updateUserProfile, upsertWeekly, createReminder } from "../lib/moduleData";
...
export default function Home(){
  ...
  return <div className="app">
    ...
    {modal&&<div className="modal-backdrop"><form className="modal" onSubmit={saveRecord}>
      <div className="section-head"><div><h3>{formMode==="reminder"?"Create reminder":formMode==="user"?"Edit user access":formMode==="comment"?"Task comment":formMode==="remark"?"Daily Job Done remark":formMode==="vehicles"?"Vehicle details":editing?`Edit ${module}`:`Create ${module}`}</h3><p className="muted">Changes are enforced against the assigned role in Supabase.</p></div><button type="button" className="btn" onClick={closeModal}>Close</button></div>
      ...
      <div className="modal-actions">{profileError&&<div className="login-error">{profileError}</div>}<button type="button" className="btn" onClick={closeModal}>Cancel</button><button className="btn primary" disabled={busy||formMode==="vehicles"&&jobVehicles.length>=(jobs.find(j=>j.id===selectedId)?.vehicles||0)}>{busy?"Saving…":formMode==="vehicles"?"Add vehicle":"Save"}</button></div>
    </form></div>}
  </div>
}
