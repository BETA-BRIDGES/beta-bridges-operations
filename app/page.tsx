...
export default function Home(){
  ...
  async function refresh(){
    if(!supabase) return;
    try{
      const [j,t,c,s,d,mc,w,u,n,r,v,e]=await Promise.all([loadJobs(profile?.role,profile?.id),loadTasks(),loadClients(),loadStock(),loadCompletions(),loadCharges(),loadWeekly(profile?.role,profile?.id),loadProfiles(),profile?.id?loadNotifications(profile.id):Promise.resolve([]),profile?.id?loadReminders(profile.id):Promise.resolve([]),loadVehicleManagement(),profile?.role==="Super Admin"?loadLegacyDeviceExceptions():Promise.resolve([])]);
      setJobs(j);setTasks(t);setClients(c);setStock(s);setCompletions(d);setCharges(mc);setWeekly(w);setUsers(u);setNotifications(n);setReminders(r);setManagedVehicles(v);setLegacyExceptions(e as import("../lib/moduleData").LegacyDeviceExceptionRecord[]);setProfileError("");
  ...
  return <div className="app">
    ...
    {modal&&<div className="modal-backdrop"><form className="modal" onSubmit={saveRecord}>
      ...
      {formMode==="assignment"?<div className="form-grid"><Field label="Assigned Field Technician" name="assignedTechnicianId" value={form.assignedTechnicianId||""} setValue={v=>setField("assignedTechnicianId",v)} options={[{value:"",label:"Unassigned"},...technicians.map(t=>({value:t.id,label:t.fullName}))]}/></div>:formMode==="technician-job"?<div>
