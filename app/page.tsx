...
import { ClientRecord, ChargeRecord, CompletionRecord, NotificationRecord, ReminderRecord, StockRecord, UserRecord, VehicleRecord, WeeklyRecord, addCompletionRemark, updateCompletion, createCharge, createClient, createCompletion, createStock, createVehicle, deliverDueReminders, deliverReminderNow, loadCharges, loadClients, loadCompletions, loadNotifications, loadProfiles, loadStock, loadReminders, loadVehicles, loadWeekly, markNotificationRead, updateCharge, updateClient, updateStock, updateUserProfile, upsertWeekly, createReminder } from "../lib/moduleData";
...
export default function Home(){
  ...
  function openUserCreate(){setModule("Administration");setEditing(false);setFormMode("create-user");setSelectedId("");setForm({fullName:"",email:"",password:""});setModal(true)}
  ...
  async function saveRecord(e:FormEvent){
    ...
    try{
      if(formMode==="reminder"){
        ...
        if(role!=="Super Admin") throw new Error("Only Super Admin can create technician accounts.");
        const {data,error}=await supabase!.functions.invoke("admin-create-user",{body:{fullName:String(form.fullName||"").trim(),email:String(form.email||"").trim(),password:String(form.password||""),role:"Field Technician"}});
        if(error) throw new Error(error.message||"Unable to create technician account.");
        if(data?.error) throw new Error(String(data.error));
        if(!data?.ok) throw new Error("Technician account creation failed.");
  ...
  return <div className="app">
    ...
    {modal&&<div className="modal-backdrop"><form className="modal" onSubmit={saveRecord}>
      <div className="section-head"><div><h3>{formMode==="reminder"?"Create reminder":formMode==="create-user"?"Create technician account":formMode==="user"?"Edit user access":formMode==="comment"?"Task comment":formMode==="remark"?"Daily Job Done remark":formMode==="vehicles"?"Vehicle details":editing?`Edit ${module}`:`Create ${module}`}</h3><p className="muted">Changes are enforced against the assigned role in Supabase.</p></div><button type="button" className="btn" onClick={closeModal}>Close</button></div>
      ...
      <div className="modal-actions">{profileError&&<div className="login-error">{profileError}</div>}<button type="button" className="btn" onClick={closeModal}>Cancel</button><button className="btn primary" disabled={busy||formMode==="vehicles"&&jobVehicles.length>=(jobs.find(j=>j.id===selectedId)?.vehicles||0)}>{busy?"Saving…":formMode==="vehicles"?"Add vehicle":formMode==="create-user"?"Create technician":"Save"}</button></div>
    </form></div>}
  </div>
}
